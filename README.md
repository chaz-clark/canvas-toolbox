...
| **Local models (Ollama)** *(no subscription, no cloud, data stays on your machine)* | **Continue.dev** — open-source agentic extension; configure with your local Ollama models | no account — set the Ollama backend in Continue's settings | [Continue Marketplace](https://marketplace.visualstudio.com/items?itemName=Continue.continue) · [Ollama](https://ollama.com) |
