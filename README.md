...
The agent-engineering side tracks the field too. The **"Hermes promotion rule"** — a friction an agent hits twice across sessions gets promoted from a one-off note into a durable [`knowledge/learned/`](lib/agents/knowledge/learned/) file — adapts [Nous Research's Hermes Agent](https://github.com/NousResearch/hermes-agent) auto-distillation pattern, rebuilt as a manual Markdown convention rather than Hermes's own automated pipeline; it's been in active use since May 2026 and is referenced throughout `lib/agents/`. Canvas Toolbox 2.0's capability-consent gate (instructor approval required when a package's declared capabilities grow) was built independently through this project's own pilot process, then checked against [Andrew Ng's OpenWorker](https://github.com/andrewyng/openworker) approval model as a second opinion — that comparison surfaced one real gap (tracked in [#343](https://github.com/chaz-clark/canvas-toolbox/issues/343): `--approve-all` has no check for whether a human is actually present in an unattended run).
