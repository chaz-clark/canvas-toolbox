...
| `lib/tools/canvas_run.py` | **Split-agent Canvas gate (opt-in).** For institutions that approve one AI vendor for university systems and not another. The only process that reads the token (from `.env.canvas`, deliberately NOT `.env`), so an agent denied that file has no credential and Canvas answers 401 — the block is **structural, not policy**. Exposes named subcommands only (`pull` / `status` / `audit` / `quality` / `push`), each mapping to exactly one toolkit invocation, so there is no argument-injection surface; default-deny on anything unlisted. `push` requires `--confirm-course <id>` matching the target. Every decision logged to `.canvas/canvas-run.log`. **Agent-neutral** — names no vendor; which agent sits on which side is local config. Scaffold starters in `scaffold/claude/` + `scaffold/env.canvas.example`. Honest limit: a guardrail against drift and accident, **not** a sandbox — a shell can defeat any client-side control; the hard control is scoping the Canvas token itself. | When an institution permits one agent to touch Canvas and not another; pairs with [`docs/split-agent-access.md`](docs/split-agent-access.md) |
