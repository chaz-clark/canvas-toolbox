...
| `lib/tools/canvas_run.py` | **Canvas access boundary (opt-in).** A plain CLI — **no AI** — for institutions that have not approved an AI tool for authenticated Canvas access. The insight: **nothing at the Canvas boundary needs intelligence** (pull = fetch-and-write-files, audit = read-and-write-a-report, push = POST), and the audit output is a **file** the agent reads from disk — so the agent never needed the network, only the *results* of a fetch. The instructor runs this script; the agent stays local. It is the only process that reads the token (from `.env.canvas`, deliberately NOT `.env`), so an agent denied that file has no credential and Canvas answers **401** — the block is **structural, not policy**, and does not depend on the agent cooperating. Named subcommands only (`pull` / `status` / `audit` / `quality` / `push`), each mapping to exactly one toolkit invocation → no argument-injection surface; default-deny on anything unlisted. `push` requires `--confirm-course <id>` matching the target (a live-course grading change re-scores real student work immediately). Every decision logged to `.canvas/canvas-run.log`. Scaffold starters in `scaffold/claude/` + `scaffold/env.canvas.example`. **Honest limit:** a guardrail against drift and accident, **not** a sandbox — a shell defeats any client-side control; the hard control is scoping the Canvas token itself. | When an institution hasn't approved your AI tool for Canvas; pairs with [`docs/canvas-access-boundary.md`](docs/canvas-access-boundary.md) |
