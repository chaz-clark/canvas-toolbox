...
- **Post-Stage-6 backlog (deferred limitations of the in-flight rubrics workstream — none block Stage 6 wiring; all are worth visiting after first real-course run reveals what actually matters)**:
  ...
  - **Stage 5 (`rubric_quality_audit.py`) heuristic calibration risks** — DON'T tune speculatively; learn from real-course noise. ~~**Highest priority deferred item**: Criterion 1 "unverified → flagged" misbehavior~~ **DONE 2026-05-21** — Criterion 1 now returns three-state (True/False/None); `None` (no CLOs) routes to a `criterion_unverified` set, NOT to `rubric_criteria_flags`, and does NOT raise `validity_flag`. New verdict `meets_criteria_unverified` (exits 0). Verified by unit tests. Lower priority calibration items (still deferred): Criterion 1 token-overlap can fire spuriously on common words (tune stopword list / threshold post-run); Criterion 2 subjective-term regex is English-only and finite (extend after first run); Criterion 3 binary test ("0 process AND ≥1 output → fail") may mis-classify legitimately-mixed rubrics; Criterion 4 `criterion_use_range` field unverified to be in `include[]=rubric` (`--probe` mode would dump one rubric's raw structure pre-run); Criterion 4 accountability detection depends on description keyword overlap; typology classifier never returns `developmental` (no heuristic); three-column single-point heuristic looks for specific labels that real Gonzalez-2017 rubrics may not use; verdict threshold (3+ flags → `needs_revision`) is arbitrary.
...
- **In-flight (uncommitted; pending v0.21.0; not yet exercised against a live course)** — Rubrics workstream + Canvas-API knowledge architecture. **Three new knowledge files (all v0.x untested):** [`rubrics_knowledge.md/.json`](lib/agents/knowledge/rubrics_knowledge.md) v0.2 (reference shape — 4-criterion backbone meta-rubric from `rubrics of rubrics.pdf`, 4 typologies analytic/holistic/single-point/developmental, AAC&U VALUE + Walvoord PTA + BYUI Assessment Services as institutional anchors, single-point/developmental exemption rules); [`canvas_api_knowledge.md/.json`](lib/agents/knowledge/canvas_api_knowledge.md) v0.1 (**strict source discipline: Canvas-authored only** — `canvas.instructure.com/doc/api/` + `instructure/canvas-lms` YARD; mental models, universal patterns, per-resource pointers); [`canvas_api_lessons_learned.md/.json`](lib/agents/knowledge/canvas_api_lessons_learned.md) v0.1 (**empirical companion** — the 16 ESL lessons migrated from this file's prior table + 7 cross-cutting patterns + audit-indicator catalog; paired-read obligation with `canvas_api_knowledge`). The "External System Lessons" section above now holds a 16-row navigation index pointing at the lessons knowledge file. **Two new audit tools (untested against live course):** [`rubric_coverage_audit.py`](lib/tools/rubric_coverage_audit.py) (Stage 4 — 6-bucket classifier: `has_rubric` / `decorative_rubric` / `missing_rubric` / `newquiz_externaltool` / `non_submittable` / `non_gradable`; 11-case classifier unit test PASS) and [`rubric_quality_audit.py`](lib/tools/rubric_quality_audit.py) (Stage 5 — per-rubric backbone meta-rubric scoring with `rubric_quality` + `rubric_criteria_flags` + `validity_flag` + `reliability_flag` + heuristic `rubric_typology`; 22-case detector unit test PASS). Both tools support `--target <env_var>` / `--course-id <id>` / `--detailed` / `--report PATH` / `--json` (machine-readable output for downstream piping) / `--allow-enrolled` (advisory only). **Two surveys in `pre_knowledge/`:** [`canvas_rubrics_api_survey.md`](lib/agents/pre_knowledge/rubrics/canvas_rubrics_api_survey.md), [`pages_api_survey.md`](lib/agents/pre_knowledge/canvas_api/pages_api_survey.md). Canvas docs site was 503 throughout authoring — every Canvas-authored fact in `canvas_api_knowledge` is GitHub-YARD-sourced, needs re-verification against canonical when reachable. Promotion to v1.0 + `knowledge/README.md` catalog entries + agent `cross_references[]` wiring deferred to Stage 6 (post-real-course-run); honest limitations + post-Stage-6 backlog logged in [`CLAUDE.md`](CLAUDE.md).
...
Canvas API has multiple non-obvious behaviors the toolkit discovered through production use. The full catalog of 16 lessons (each with behavior, why-it-matters, defense, and provenance) has been migrated to the dedicated knowledge file [`lib/agents/knowledge/canvas_api_lessons_learned.md`](lib/agents/knowledge/canvas_api_lessons_learned.md) (2026-05-21). It pairs with [`lib/agents/knowledge/canvas_api_knowledge.md`](lib/agents/knowledge/canvas_api_knowledge.md), which holds the Canvas-documented surface (strict source discipline: only Instructure-authored docs in that file; only empirical findings in the lessons file).

Always read both knowledge files when planning a Canvas write or audit. The 16 lessons are abbreviated here as a navigation index — full content + defense citations live in the knowledge file:
...
| `lib/tools/rubric_coverage_audit.py` | **Read-only** rubric coverage audit per assignment: classifies as `has_rubric` / `decorative_rubric` (rubric attached but `use_rubric_for_grading=false`, [L9](lib/agents/knowledge/canvas_api_lessons_learned.md)) / `missing_rubric` (the gap) / `newquiz_externaltool` ([L8](lib/agents/knowledge/canvas_api_lessons_learned.md)) / `non_submittable` / `non_gradable`. `--target <env_var>` (default `CANVAS_COURSE_ID`) or `--course-id <id>` (literal). `--detailed` (per-assignment with module location). `--report PATH` writes markdown. Exits non-zero on `missing_rubric` or `decorative_rubric` findings. Advisory safety guard. Verification limit (honest): static + argparse + 11-case classifier unit-test only — no live course in this repo. Stage 4 of the rubrics workstream. | Pre-semester rubric coverage check; pairs with [`rubrics_knowledge.md`](lib/agents/knowledge/rubrics_knowledge.md) |
| `lib/tools/rubric_quality_audit.py` | **Read-only** per-rubric backbone meta-rubric scoring: for each rubric in the course, scores all 4 backbone criteria (Criteria Alignment, Rating Levels, Process-Oriented, Points & Weights) per [`rubrics_knowledge.md`](lib/agents/knowledge/rubrics_knowledge.md). Emits `rubric_quality` ∈ {meets_criteria / partial / needs_revision / absent} + `rubric_criteria_flags` + `validity_flag` + `reliability_flag` + heuristic `rubric_typology` (analytic / holistic / single_point / developmental / unknown). Honors single-point and developmental exemption rules. Heuristics: subjective-language scan (Criterion 2), process-vs-output term lists (Criterion 3), `criterion_use_range=true` API signal + accountability-criterion weighting check (Criterion 4), token-overlap CLO match (Criterion 1, falls back to "unverified" when CLOs aren't fetchable). All flags are reviewable — heuristic, false-positives expected. Same `--target`/`--course-id`/`--detailed`/`--report PATH`/`--allow-enrolled` shape as `rubric_coverage_audit.py`. Exits non-zero on partial or needs_revision findings. Verification limit (honest): static + argparse + 22-case unit-test across all 5 detectors and both exemption rules — no live course in this repo. Stage 5 of the rubrics workstream. | After running `rubric_coverage_audit.py` to triage rubric quality; pairs with [`rubrics_knowledge.md`](lib/agents/knowledge/rubrics_knowledge.md) and [`outcomes_quality_knowledge.md`](lib/agents/knowledge/outcomes_quality_knowledge.md) |
