...
- `grader_fetch_resubmissions.py` — maintenance tool (issue #NEW): detects Canvas resubmissions where `submitted_at > graded_at` (student resubmitted after receiving a grade, typically after "incomplete"). Canvas marks these as "needs grading" but they're easy to overlook because they already have a grade. Generates FERPA-safe review reports (user_id only) with SpeedGrader links. Run `--all` to scan all assignments with `needs_grading_count > 0`. Real-world: ITM327 Spring 2026 had 21 resubmissions across 10 assignments go unnoticed for weeks until this tool surfaced them. Pairs with `grader_push.py` for grade submission (AI disclosure tag added automatically).
