...
| **Local models (Ollama)** *(no subscription, no cloud, data stays on your machine)* | **Continue.dev** (preferred) — or **Cline** as an alternative — both open-source agentic extensions; configure with your local Ollama models | no account — set the Ollama backend in the extension's settings | [Continue Marketplace](https://marketplace.visualstudio.com/items?itemName=Continue.continue) · [Cline Marketplace](https://marketplace.visualstudio.com/items?itemName=saoudrizwan.claude-dev) · [Ollama](https://ollama.com) |
...
> 🦙 **About Ollama + Continue.dev / Cline:** Both are open-source and fully agentic — they read files, run terminal commands, and edit code, the same workflow as the cloud-based extensions above. **Continue.dev** is the preferred first pick (Apache 2.0; broader adoption; longer history; more stable backend abstraction). **Cline** is a strong alternative — newer but very capable for the same agentic workflow; some operators prefer its UX. Both are local-first by design: nothing leaves your machine, which makes either a good fit for institutions with strict FERPA policies or cost-conscious workflows. The trade-off is model capability — today's local code models (e.g. `qwen2.5-coder`, `deepseek-coder-v2`, `codestral`) handle canvas-toolbox's deterministic + structural work well, but typically need extra calibration for nuanced prose grading (voice-anchored comments, critical-thinking judgment) compared to Claude / GPT-4. Start with a recent code-focused model; tune from there.
