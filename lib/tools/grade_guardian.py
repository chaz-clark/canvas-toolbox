...
#
# `/quizzes/<anything>/submissions`: a Classic Quiz grade can be changed WITHOUT
# ever touching posted_grade or an /assignments/.../submissions URL —
# grader_quiz_clear_pending.py zeroes a pending question's score via
# PUT .../quizzes/{qid}/submissions/{id} with {"quiz_submissions": [{"questions":
# {q: {"score": 0}}}]}. Found by auditing every sanctioned grade/comment writer's
# actual payload against this pattern — this shape wasn't covered, so a
# hand-written bypass mimicking it would have gone undetected.
